...
import 'package:mavencliq/Presentation/Screens/Mentee/Screens/Auth/ForgotPass/mentee_forgot_password_screen.dart';
...
class MenteeSignInScreen extends StatefulWidget {
  const MenteeSignInScreen({super.key});

  @override
  State<MenteeSignInScreen> createState() => _MenteeSignInState();
}

class _MenteeSignInState extends State<MenteeSignInScreen> {
  ...
  navigateToMenteeForgotpassScreen() {
    Navigator.push(context, MaterialPageRoute(
      builder: (context) {
        return MenteeForgotPasswordScreen();
      },
    ));
  }

  onMenteeSignInButtonTap() {
    _removeFocus();
    String emailTxt = menteeEmailController.text.trim();
    String passTxt = menteePassController.text.trim();
    final bool? isValid = menteeFormKey.currentState?.validate();

    if (isValid == true) {
      menteeAuthBloc.add(MenteeSignInRequestEvent(emailTxt, passTxt));
    } else {
  ...
  // verifyMenteeEmail() {
  //   var userToken =
  //       'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6InNhbmpheUB0ZWNodXouY29tIiwiaWF0IjoxNjg0MTM2MzY1LCJleHAiOjE2ODQyMjI3NjV9.Y93N1qC__n8S-irmMj77YfXrEDpoFpq0nqLtpo9Tm6c';
  //   menteeAuthBloc.add(MenteeVerifyEmailEvent(userToken));
  // }
    ...
    return Scaffold(
      ...
      body: BlocConsumer<MenteeAuthBloc, MenteeAuthState>(
        listener: (context, state) {
          if (state is MenteeSignInCompletedState) {
          ...
          } else if (state is MenteeSignInFailedState) {
            errorMsg = state.response.message;
          } else if (state is MenteeVerifyEmailCompletedState) {
            AppUtilities(context).showTextSnackBar(state.response.message);
          } else if (state is MenteeVerifyEmailFailedState) {
            AppUtilities(context).showTextSnackBar(state.response.message);
          }
        },
        builder: (context, state) {
          if (state is MenteeSignInLoadingState) {
            return SizedBox(
              child: Center(
